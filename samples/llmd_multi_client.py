...
"""
    ...
    try:
        ...
        handler1 = interface.get_dbus_method(
            "configure",
            dbus_interface="com.intel.llmd",
        )(*config1.values())

        handler2 = interface.get_dbus_method(
            "configure",
            dbus_interface="com.intel.llmd",
        )(*config2.values())
    
        print(f"supported models: {models_supported}")
        print(f"handler1 is {handler1}")
        print(f"handler2 is {handler2}")

        result1 = interface.get_dbus_method(
            "generate",
            dbus_interface="com.intel.llmd",
        )(handler1, text, timeout=timeout_seconds)
        print(f"result1: {result1}")

        result2 = interface.get_dbus_method(
            "generate",
            dbus_interface="com.intel.llmd",
        )(handler2, text, timeout=timeout_seconds)
