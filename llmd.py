...
class GenAI:
    ...
    def __init__(self, model_id, temperature=0.1, top_k=50, top_p=0.9, repetition_penalty=1.1):
        ...
        self.ov_config = {"PERFORMANCE_HINT": "LATENCY", "NUM_STREAMS": "1", "CACHE_DIR": ""}
        self.model_name = self.model_configuration["model_id"]

        self.text_processor = self.model_configuration.get("partial_text_processor", self.default_partial_text_processor)
        self.max_new_tokens = 128
        self.tok = AutoTokenizer.from_pretrained(self.model_dir, trust_remote_code=True)
        ...
        self.history_template = self.model_configuration.get("history_template")
        self.current_message_template = self.model_configuration.get("current_message_template")
        ...
        self.tokenizer_kwargs = self.model_configuration.get("tokenizer_kwargs", {})
    ...
    def convert_history_to_token(self, history: List[Tuple[str, str]]):
        ...
        input_token = self.tok(text, return_tensors="pt", **self.tokenizer_kwargs).input_ids
        return input_token
    ...
    def LLMGenerator(self, history):
        ...
        if input_ids.shape[1] > 2000:
            history = [history[-1]]
            input_ids = self.convert_history_to_token(history)
        streamer = TextIteratorStreamer(self.tok, timeout=30.0, skip_prompt=True, skip_special_tokens=True)
...
class LLMDService(dbus.service.Object):
    ...
    def __init__(self):
        ...
        """
        bus_name = dbus.service.BusName('com.intel.llmd', bus=dbus.SessionBus())
